...
function parseStopToken(
  ...
  if (pctMatch)
    return { color: pctMatch[1].trim(), left: parseFloat(pctMatch[2]) };
...
export function parseGradient(value: string): ParsedGradient {
  ...
  if (isLinear) {
    ...
    const looksLikeColor =
      first.startsWith("#") ||
      first.startsWith("rgb") ||
      first.startsWith("hsl");
  ...
  const colors = colorParts.map((part, i) =>
    parseStopToken(part, i, colorParts.length),
  );
  colors.forEach((c) => {
    if (!parseColor(c.color)) c.color = "#000000";
  });
...
export function buildGradientString(
  ...
  const stopStr = sorted
    .map((s) => `${s.color} ${Math.round(s.left)}%`)
    .join(", ");
